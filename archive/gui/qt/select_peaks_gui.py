...
class SelectPeaksWindow(QWidget):
    def __init__(self):
        super().__init__()
        # Window for selction of peaks region for fitting.
        layout = QVBoxLayout()
        ...
        self.setWindowIcon(QIcon("../../../doc/select.png"))
    ...
    def cell(self, var=""):
        ## Creates cell for table text.
        item = QTableWidgetItem()
        item.setText(var)
        return item

    def createTable(self):
        ## Creates table for input arguments.
        ...
        contents = [
            [
                "Files path:",
                "/asap3/fs-bmx/gpfs/regae/2022/data/11015669/scratch_cc/rodria/processed/centered/20221114/20221114_Au",
            ],
            ["Number of peaks per file to follow:", "1"],
            [
                "Peak(s) file:",
                "/asap3/fs-bmx/gpfs/regae/2022/data/11015669/scratch_cc/rodria/processed/calib/2022114_Au_free_peak_0.txt",
            ],
        ]
    ...
    def update_peak_map(self):
        ## Update of peak positions in table.
    ...
    def on_press(self, event):
        ...
        self.peaksRangeTableWidget.setItem(
            index[0][0], index[1][0] + (position_in_table * self.number_of_peaks), item
        )
        x = [round(event.xdata)]
        y = [round(event.ydata)]
    ...
    def createPeaksRangeTable(self):
        ## Create table for peak range positions
        ...
        self.peaksRangeTableWidget.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch
        )
    ...
    def _mark_peaks_button_clicked(self) -> None:
        ## Manages clicks on the 'Mark peaks' button.
    ...
    def _open_button_clicked(self) -> None:
        ...
        if self.count_clicks_open == 0:
            self.label_2 = QLabel(
                f"/{round((int(self.init_table[4])/int(self.init_table[5]))*(int(self.init_table[9])-int(self.init_table[7])+1))}"
            )
    ...
    def _export_file_button_clicked(self) -> None:
        ## Manages clicks on the 'Export' button.
    ...
    def _next_file_button_clicked(self) -> None:
        ## Manages clicks on the 'Next' button.
    ...
    def _confirm_button_clicked(self) -> None:
        ## Manages clicks on the 'Confirm' button.
        self.confirmPeaksButton.setCheckable(True)
        self.confirmPeaksButton.setChecked(False)
        args = [
            "-n",
            f"{self.number_of_peaks}",
            "-i",
            self.input_order,
            "-a",
            self.samePeaksCheckBox.isChecked(),
        ]
        columns = self.peaksRangeTableWidget.columnCount()
        max_column = 0
        for j in range(columns):
    ...
    def show_plots(self, raw_args=None):
        ...
        ## Plot azimuthal integration scpetra.

...
from convert_images import apply_calibration
import center_finding
from utils import sum_nth, radial_average, sum_nth_mask, baseline_als, calc_fwhm
...
def main():
    ...
    sum_img_masked=sum_nth_mask(np.array(mask_ctr_img_lst),1,0)
    print(f'Azimuthal integration')
    ...
    #plt.plot(rad_sub)
    #plt.show()
    peaks, half=calc_fwhm(rad_sub,6,threshold=100,distance=5, height=5, width=3)

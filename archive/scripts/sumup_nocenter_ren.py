# -*- coding: utf-8 -*-
"""
   Function DEPRECATED. Include gaps not precise. Better import crystfel geometry file.
"""
import time
import h5py
import numpy
...
def main():
    ...
    with h5py.File(args.input, "r") as f:
        ...
        with Pool(10) as p:
            d=p.map(apply_calibration, f["data_f000000000000"][0][begin:begin+n_frames]+f["data_f000000000000"][1][begin:begin+n_frames])
    ...
    plt.imshow(sum_img,vmax=1e4)
    plt.show()
    center=center_finding.calc_center_cc_par(sum_img)
    acc_corr_image=center_finding.include_gaps_np(sum_img)
    new_data, shift=center_finding.bring_center_to_point(acc_corr_image, center)
    new_mask, shift=center_finding.bring_center_to_point(mask, center)
    ctr_img_lst=new_data 


    print(f'Azimuthal integration')
    rad_signal=radial_average(new_data, None)

    print(f'Azimuthal integration')
    #rad_signal_masked=radial_average(sum_img, sum_img_masked)
    masked_data_final=new_data*new_mask
    plt.imshow(masked_data_final,vmax=1e4)
    plt.show()
    rad_signal_masked=azimuthal_average(masked_data_final, center=(515,532),angular_bounds=(0,360),trim=False)
    rad_signal_cut=[]
    beam_cut=40
    ...
    plt.plot(rad_sub)
    plt.show()
    peaks, half=utils.calc_fwhm(rad_sub,6,threshold=250,distance=5, height=5, width=3)
    ...
    #angles, peak_positions=utils.check_ellipticity(masked_data_final)

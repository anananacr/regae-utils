...
def main():
    ...
    parser.add_argument("-s", "--target_sample", type=str, action="store",
        help="sample")
    parser.add_argument("-j", "--j_sol", type=float, action="store",
        help="j sol 67 in amperes")
    parser.add_argument("-f", "--f_cal", type=float, action="store",
        help="calibration factor A.U.")
    parser.add_argument("-w", "--_lambda", type=float, action="store",
        help="wavelength in angstroms")
    parser.add_argument("-u", "--unit", type=str, action="store",
        help="x axis parameter x/q/two_theta")
    ...
    for idx,i in enumerate(reflections_list):
        ...
        if target_sample['group']=='cubic':
            ...
            b=target_sample['b']
            c=target_sample['c']        
            q=math.sqrt((i[0]**2/a**2)+(i[1]**2/b**2)+(i[2]**2/c**2))
            ...
            q=math.sqrt((i[0]**2)/((a**2)*(np.sin(beta)**2))+((i[1]**2)/(b**2))+(i[2]**2)/((c**2)*(np.sin(beta)**2))-(2*i[0]*i[1]*np.cos(beta)/(a*c*(np.sin(beta)**2))))

        elif target_sample['group']=='hexagonal':
            a=target_sample['a']
            c=target_sample['c']        
            q=math.sqrt(((4/3)*(i[0]**2+i[0]*i[1]+i[1]**2)/a**2)+(i[2]**2/c**2))

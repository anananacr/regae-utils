...
center = [[581, 562], [581, 562], [583, 562], [585, 560], [586, 558], [587, 555], [587, 554], [587, 552], [587, 549], [587, 545], [587, 540]]
height = [100,100,100,100,100,100,100,100,50, 5,5]

max_frames=11


def gaussian_lin(x: np.ndarray, a: float, x0: float, sigma: float, m: float, n: float) -> np.ndarray:
    """
    Gaussian function.
...
def get_minimum(file_path:str, output:str):
    ...
    for frame in frames:
        ...
        fwhm_over_radius.append(fit_gaussian(bins, counts, x))
        ax.scatter(x,y, c='r', label=f'Sol67 {round(frame*0.3,1)}A\n FWHM/x0:{round(fwhm_over_radius[-1],3)}')

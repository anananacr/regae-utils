...
class RadialPlotsWindow(QWidget):
    def __init__(self):
        ...
        self._lambda=1e10*constants.h * constants.c / math.sqrt((self.beam_energy)**2+(2* self.beam_energy * constants.electron_mass * (constants.c**2)))
    ...
    def _open_button_clicked(self) -> None:
        ...
        f= h5py.File(f'{self.Root}/{self.Date}/{self.file_name}{self.Norm}_time_scans.h5', "r")
        ...
        args=['-i',f'{self.Root}/{self.Date}/{self.file_name}{self.Norm}_time_scans.h5','-o',f'{self.Root}/../plot/{self.Date}_{self.Label}_time_scans']
        times=[]
        rows=self.tableWidget.rowCount()
        for i in range(rows):
    ...
    #def check_point_is_in_plot(event)
        ###TODO

    def on_press(self,event):
        text=self.e1.text()
        #check_point_is_in_plot(event)
        if self.unit=='x':
    ...
    def gold_peaks_pos(self,j_sol):
        ...
        ##unit cell parameter gold in angstroms
        a=407.3*1e-2
    ...
    def plot_rad(self,raw_args=None):
        ...
        parser.add_argument("-t", "--time", action='append',
            help="time points in scan plots")
            ...
            if self.mode==1:
                ...
                if self.hideOffCheckBox.isChecked()==False:
                    ...
                    if self.normRadCheckBox.isChecked()==True:
                        peak_pos=self.text_to_list(self.e1.text()[1:]+' ',n_dec=5)
                        x=x/peak_pos[idx]
                    ax.plot(x,norm_rad_0, ':',color=colors[idx],label=f'{i}ps laser off',linewidth=2)
                ...
                if self.hideOnCheckBox.isChecked()==False:
                    ...
                    ax.plot(x,norm_rad_0, '.-',label =f'{i}ps laser on',color=colors[idx],linewidth=2)
                ...
                ax.plot(x,norm_rad_0, '.-',color=colors[idx],label=f'{i}A',linewidth=2)            
...
class ResultsWindow(QWidget):
    def __init__(self):
        ...
        self.normComboBox.addItems(['no','total_sum'])
    ...
    def createTable(self):
        ...
        contents=[["Root path:","/asap3/fs-bmx/gpfs/regae/2022/data/11016614/processed/average"],["Raw files date:","20221114"],["Label:","Au"], ["Peak number:","0"]]
    ...
    def _open_button_clicked(self) -> None:
        ...
        args=['-i',scan_file,'-n',f'{args[3]}','-o',f'{self.Root}/../plot/{self.Date}_{self.Label}{self.Norm}','-fi',f'{int(self.config_delay[1])}']
        self.plot_scan(args)
    ...
    def plot_scan(self,raw_args=None):
        ...
        parser.add_argument("-i", "--input", type=str, action="store",
            help="hdf5 input image")
        parser.add_argument("-fi", "--first", type=str, action="store",
            help="hdf5 input image")
        parser.add_argument("-n", "--n_peak", type=int, action="store",
        ...
        if self.mode==1 and self.hideSubCheckBox.isChecked()==False:
            for idx, i in enumerate(rad_0):
                try:
                    ...
                    sub_err.append(math.sqrt((rad_0_on_err[idx])**2 + (rad_0_err[idx])**2))
                except ValueError:
                    sub.append(0)
                    sub_err.append(0)                       
        ...
        if self.mode==0 or (self.mode==1 and self.hideOnCheckBox.isChecked()==False):        
            ax1.scatter(label,rad_0_on,color='blue',marker='o')
            ax1.errorbar(label,rad_0_on, yerr=rad_0_on_err, color='blue',label='laser_on')

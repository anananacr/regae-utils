...
def resolution_rings(data, center, rings, Imax=1000):
    ...
    pos = ax.imshow(data, cmap='jet', origin='upper', interpolation=None, norm=color.LogNorm(0.1,Imax))
...
def ring_deviation(data,radius):
    ...
    intensities = []
    bin_size = 1
    mask = np.where(np.greater(R, radius - bin_size) & np.less(R, radius + bin_size) & (data!=0))
    intensities= data[mask]
    deg=[]
    for i in range(len(mask[0])):
        theta=np.rad2deg(np.arctan2(mask[0][i] - center[1], mask[1][i] - center[0])) + 180
...
def ellipse_measure(data,beam_cut=0):
    ...
    for i in range(15,350,5):
        rad_signal_masked=azimuthal_average(data, center=(515,532),angular_bounds=(i-15,i+5),trim=True)
        rad_signal_cut=[]
        ...
        peaks, half=calc_fwhm(rad_sub,4,threshold=400, height=1, width=5, distance=8)
        intensity=[]
...
def check_ellipticity(data, el_rat=1, el_ang=0):
    ...
    for i in range(19,359,1):
        rad_signal_masked=azimuthal_average(data, center=(515,532),angular_bounds=(i-19,i+1),trim=True)
        rad_signal_cut=[]
        ...
        peaks, half=calc_fwhm(rad_sub,-1,threshold=100, height=1, width=3, distance=5)
        intensity=[]
        for j in range(len(peaks)):
            intensity.append(rad_sub[peaks[j]])
        #plt.plot(rad_sub)
    ...
    rad_range = (25, 100)
    powder_polar = np.histogram2d(peak_positions, angle, bins=[np.linspace(*rad_range, 200), np.linspace(0, 360, 360)])
    ...
    print(f'Median ellipticity variance: {np.nanmedian((np.nanvar(powder_polar[0], axis=1)/np.nanmean(powder_polar[0],axis=1))):.2f} \n'
...
def calc_new_coord(x0,y0,el_rat,el_ang):
    ...
    x1, y1 = 1/el_rat**.5*(c*x0 - s*y0), el_rat**.5*(s*x0 + c*y0)

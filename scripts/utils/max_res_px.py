...
def main(raw_args=None):
    ...
    parser.add_argument("-d", "--distance", type=float, action="store",
        help="detector ditance in meters")
    parser.add_argument("-r", "--resolution", type=float, action="store",
        help="resolution in angstroms")
    parser.add_argument("-e", "--energy", type=int, action="store",
    ...
    max_resolution_in_px=1.0 * _last_pixel_size * (_last_detector_distance)* np.tan(2.0* np.arcsin(lambda_ / (2.0 * resolution_in_a)))

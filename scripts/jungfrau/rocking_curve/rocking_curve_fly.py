...
def main(raw_args=None):
    ...
    description="Plot summed intensity over a peak square according to the angle of rotation in fly-scan mode.")
    parser.add_argument("-i", "--input", type=str, action="store",
    help="H5 input master file")
    parser.add_argument("-s", "--start_frame", type=int, action="store",
    help="starting frame in the interval of images measured")
    parser.add_argument("-e", "--end_frame", type=int, action="store",
    help="ending frame in the interval of images measured")
    parser.add_argument("-p_x0", "--p_x0", type=int, action="store",
    help="first x position (column) of the peak position square on wich the sum will be calculated")
    parser.add_argument("-p_x1", "--p_x1", type=int, action="store",
    help="las x position (column) of the peak position square on wich the sum will be calculated")
    parser.add_argument("-p_y0", "--p_y0", type=int, action="store",
    help="first y position (row) of the peak position square on wich the sum will be calculated")
    parser.add_argument("-p_y1", "--p_y1", type=int, action="store",
    ...
    peak_image=np.zeros((n_frames, peak_pos_y[1]-peak_pos_y[0], peak_pos_x[1]-peak_pos_x[0]), dtype=np.float64)
    for idx,i in enumerate(d):
        peak_image[idx]=i[peak_pos_y[0]:peak_pos_y[1], peak_pos_x[0]:peak_pos_x[1]]

...
def main(raw_args=None):
    ...
    """
    _resolution_rings_in_index: List[float] = ["[111]", "[002]", "[022]", "[113]"]
    a = 4.08
    ...
    lambda_ = (
        ...
        * constants.c
        / math.sqrt(
            (beam_energy) ** 2
            + (2 * beam_energy * constants.electron_mass * (constants.c**2))
        )
    )
    ...
    _resolution_rings_in_a = [
        a / math.sqrt(int(i[1]) ** 2 + int(i[2]) ** 2 + int(i[3]) ** 2)
        for i in _resolution_rings_in_index
    ]
    ...
    print(
        "clen", _last_detector_distance, "coff", _last_coffset, resolution_rings_in_pix
    )
    ...
    for idx, i in enumerate(resolution_rings_in_pix):
        ...
        if idx > 0:
            plt.text(
                center[0] - 35 + int(i),
                center[1] - 35 + int(i),
                f"{_resolution_rings_in_index[idx-1]}",
                color="k",
                fontsize=8,
            )

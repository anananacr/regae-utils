...
def main(raw_args=None):
    ...
    size=np.flip(np.arange(0,82,1))
    start=-1*(240-227.5)*6.66
    finish=(227.5-220)*6.66
    label=np.arange(start,finish+1,3.33)
    ...
    color=['orange','r','g','b','g','g']
    count=0
    print('len'+str(len(label)))
    for idx,i in enumerate(size[:]):
        print(idx)
        if idx%2==0:
            pos=int(round((idx/2),0))
            x=[label[pos],label[pos],label[pos],label[pos],label[pos],label[pos]]
        else:
            pos=int(round(((idx-1)/2),0))
            x=[label[pos],label[pos],label[pos],label[pos],label[pos],label[pos]]
        ...
        if i%2==0:
            ...
            for j in range(3,4):
                ...
                if count==0:
                    ax1.scatter(x[j],norm_int, color=color[j],label =f'peak_{j}_off',marker='o')
            ...
            #print(rad_1[j], norm_rad_1)
                ...
                if count==0:
                    ax2.scatter(x[j],norm_fwhm, color=color[j],label=f'peak_{j}_off',marker='o')
            ...
            for j in range(3,4):
                ...
                norm_int_err=round((rad_0_err[j]/norm_rad_0[j]),6)
                if count==1:
                    ax1.scatter(x[j],norm_int, color=color[j+1],label =f'peak_{j}_on',marker='o')
            ...
            #print(rad_1[j], norm_rad_1)
                ...
                norm_fwhm_err=round((rad_1_err[j]/norm_rad_1[j]),6)
                if count==0:
                    ax2.scatter(x[j],norm_fwhm, color=color[j+1],label=f'peak_{j}_on',marker='o')
                    ax2.errorbar(x[j],norm_fwhm,yerr=norm_fwhm_err,color=color[j+1])
                else:

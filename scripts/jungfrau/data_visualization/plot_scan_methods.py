"""
Function that plots together intensity and FWHM of Au powder peaks for different centering methods (no centering, center of mass, cross correlation).
"""
...
color = ["r", "g", "b"]
count = 0
## Set number of peaks to plot
n_peaks = 1
for k in range(n_peaks):
    for i in size:
        x = [i, i, i]
        ## Set correct data path
        file_path = f"path/to/file_{i}.h5"
        ...
        for j in range(len(color)):
            if count == 0:
                ax1.scatter(
                    x[j], rad_0[j], color=color[j], label=f"peak_{j}", marker="."
                )
            else:
        ...
        for j in range(len(color)):
            if count == 0:
                ax2.scatter(
                    x[j], rad_1[j], color=color[j], label=f"peak_{j}", marker="."
                )
            else:

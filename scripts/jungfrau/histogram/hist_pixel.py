...
def main(raw_args=None):
    ...
    parser.add_argument(
        "-px", "--pixel_x", type=int, action="store", help="x/collum pixel position"
    )
    parser.add_argument(
        "-py", "--pixel_y", type=int, action="store", help="y/row pixel position"
    )
    parser.add_argument(
    ...
    thr = 10
    values = [
        i[args.pixel_y, args.pixel_x]
        for i in data
        if i[args.pixel_y, args.pixel_x] > thr
    ]
